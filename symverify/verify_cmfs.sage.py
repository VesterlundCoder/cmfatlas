...
# This file was *autogenerated* from the file verify_cmfs.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_30 = Integer(30); _sage_const_60 = Integer(60); _sage_const_4 = Integer(4); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_72 = Integer(72); _sage_const_0p0 = RealNumber('0.0'); _sage_const_300 = Integer(300); _sage_const_38 = Integer(38); _sage_const_100 = Integer(100); _sage_const_120 = Integer(120)
import sqlite3
import json
import sys
import os
...
_args = sys.argv[_sage_const_1 :]

def _get_arg(flag, default=None):
    try:
        idx = _args.index(flag)
        return _args[idx + _sage_const_1 ]
    except (ValueError, IndexError):
        return default

LIMIT      = int(_get_arg('--limit', _sage_const_0 )) or None
FILTER_ID  = int(_get_arg('--id',    _sage_const_0 )) or None
...
def verify_poly_cmf(f_str, fbar_str):
    ...
    b    = R2(g.subs({m: R2(_sage_const_0 )}) * gbar.subs({m: R2(_sage_const_0 )}))   # b(k)
    b1   = R2(b.subs({k: k + _sage_const_1 }))                            # b(k+1)
    a    = R2(g - gbar.subs({k: k + _sage_const_1 }))                     # a(k,m)
...
def verify_explicit_cmf(mats_dict):
    ...
    for ii in range(len(axes)):
        for jj in range(ii + _sage_const_1 , len(axes)):
...
if FILTER_ID is not None:
    rows = [r for r in rows if r[_sage_const_0 ] == FILTER_ID]
if FILTER_SRC is not None:
    rows = [r for r in rows if
            FILTER_SRC.lower() in (json.loads(r[_sage_const_2 ]) or {}).get('source_category','').lower()]
if FILTER_MODE == 'poly':
    rows = [r for r in rows if (json.loads(r[_sage_const_2 ]) or {}).get('f_poly','')]
if FILTER_MODE == 'explicit':
    rows = [r for r in rows if (json.loads(r[_sage_const_3 ]) or {}).get('matrices')]
if LIMIT:
    rows = rows[:LIMIT]
...
results   = []
n_pass = n_fail = n_skip = n_error = n_timeout = _sage_const_0 
...
for row in rows:
    ...
    rec = {
        ...
        "result": None, "detail": "", "time_s": _sage_const_0p0 ,
        "mode": None,
    }
    ...
    if f_poly and fbar_poly:
        ...
        try:
            ...
            rec["detail"] = detail
            if flat: n_pass += _sage_const_1 
            else:     n_fail += _sage_const_1 
        except TimeoutError:
            rec["result"] = "TIMEOUT"
            rec["detail"] = f"Exceeded {TIMEOUT_S}s"
            n_timeout += _sage_const_1 
        except Exception as ex:
            rec["result"] = "ERROR"
            rec["detail"] = str(ex)[:_sage_const_300 ]
            n_error += _sage_const_1 

    elif explicit and isinstance(explicit, dict) and len(explicit) >= _sage_const_2 :
        rec["mode"] = "explicit"
        try:
            def _run():
    ...
    poly_lbl = f_poly[:_sage_const_38 ] if f_poly else (f"{dim}D explicit [{','.join(list(explicit.keys())[:_sage_const_3 ])}]" if explicit else "—")
...
print("=" * _sage_const_72 )
print(f"  TOTAL   : {total}")
print(f"  ✓ PASS  : {n_pass}  ({_sage_const_100 *n_pass//max(total,_sage_const_1 )}%)")
print(f"  ✗ FAIL  : {n_fail}")
...
if fails:
    ...
    for r in fails:
        print(f"  #{r['id']}  [{r['result']}]  {r['detail'][:_sage_const_120 ]}")
    print()
...
with open(REPORT_JSON, "w") as fh:
    json.dump(report, fh, indent=_sage_const_2 , cls=_SageEncoder)
...
with open(REPORT_TXT, "w") as fh:
    ...
    for r in results:
        ...
        fh.write(f"       {r['detail'][:_sage_const_120 ]}\n\n")
