...
async def lifespan(app: FastAPI):
    ...
    # ── Auto-ingest gauge_agents store files if DB is small (< 1000 CMFs) ──
    _cmf_count = _sq.connect(DB_PATH).execute("SELECT COUNT(*) FROM cmf").fetchone()[0]
    if _cmf_count < 1000:
        ...
        try:
            ...
            _scache: dict = {}
            def _get_series(agent, dim):
                k = (agent, dim)
                if k in _scache: return _scache[k]
                sn = f"Gauge Agent {agent} {dim}\u00d7{dim}"
                sr = _icc.execute("SELECT id FROM series WHERE name=? AND project_id=?", (sn, _pid)).fetchone()
                if sr:
                    _scache[k] = sr["id"]; return sr["id"]
                _icc.execute(
            ...
            _log2.getLogger("uvicorn.error").warning(f"[auto-ingest] FAILED: {_e}")
